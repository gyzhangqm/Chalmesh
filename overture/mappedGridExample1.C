...
main()
{
  ...
  PlotStuff ps(TRUE,"mappedGridExample1");      // create a PlotStuff object
  PlotStuffParameters psp;                      // This object is used to change plotting parameters
    
  String answer;
  String menu[] = { "contour",                  // Make some menu items
		    "stream lines",
		    "grid",
		    "read command file",
		    "save command file",
  ...
  for(;;)
  {
    ...
    if( answer=="contour" )
    {
      ...
      ps.contour(u,psp);                        // contour/surface plots
    }
    else if( answer=="grid" )
      ps.plot(mg,psp);                          // plot the grid
    else if( answer=="stream lines" )
      ps.streamLines(u);                        // streamlines
