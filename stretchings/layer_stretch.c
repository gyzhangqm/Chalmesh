...
static void 
layer_stretch_to_uniform(real r_stretch, generic_stretching *stretch_ptr, 
			 real *r_uniform, real *duni_dstr, real *d2uni_dstr2 );
static void 
...
static void 
compute_layer_stretch_info( generic_stretching *stretch_ptr );
static void 
...
static void *
copy_layer_stretch( void *stretch_data_ptr );
/* end */
...
/* assign all fields of the generic stretching structure */
  ...
  stretch_ptr->stretch_to_uniform = layer_stretch_to_uniform;
...
/* setup the stretching parameters */
  compute_layer_stretch_info( stretch_ptr );
}
...
/* default strength */
    if (info->n_tanh == 2)
      info->strength[i] = 0.25;
    else if (i==1 || i==info->n_tanh)
      info->strength[i] = 0.0;
    else
      info->strength[i] = 0.5/(info->n_tanh - 2);
...
/* do a few newton iterations to get the exact solution */
  r_str = r_stretch0;
  for (i=1; i<=5; i++){
    layer_stretch_to_uniform( r_str, stretch_ptr, &r_uni, &duni_dstr, &d2uni_dstr2 );
...
/* use the fail safe bisection method instead */
...
/* compute the scale */
  info->scale = 1.0;
  layer_stretch_to_uniform( 1.0, stretch_ptr, &r_uniform, &duni_dstr, &d2uni_dstr2 );
  info->scale = 1.0/r_uniform;

/* compute the piecewise linear approximation to the stretching function */
  for (i=1; i<=info->n_tanh; i++){
    layer_stretch_to_uniform( info->focus[i], stretch_ptr, &r_uniform, &duni_dstr, 
			     &d2uni_dstr2 );
